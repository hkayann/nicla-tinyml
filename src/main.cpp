...
#define ACC_SENSOR_UPDATE_INTERVAL                (50) 
#define MAG_SENSOR_UPDATE_INTERVAL                (50)
#define GYRO_SENSOR_UPDATE_INTERVAL               (50)
...
constexpr int kTensorArenaSize = 7490; // in bytes;
...
void loop() {
  ...
  //     Serial.readBytes((char*)&floatValues[floatCount], sizeof(float));
  //     floatCount++;
  ...
  // if (floatCount == numRows * numFloatsPerRow) {
  //   // Assuming output tensor is a 1D array of floats
  ...
  //     // Consider adding some error handling or recovery here
  //   } else {
  ...
  //     // Echo back the model's output
  //     for (int i = 0; i < N; i++) {
  //       Serial.write((const unsigned char*)&output_data[i], sizeof(float));
  //     }
  //   }

  //   delay(100); // Optional: additional delay after sending back data
